...
class TestTutorial:
    ...
    def test_tutorial_non_flat_data(self):
        ...
        nn.plot_cost_function(output_folder=Path("./tests/test_output/test_tutorial"))
        nn.predict(validation_training)
        nn.plot_fitted_line(
            validation_target, Path("./tests/test_output/test_tutorial")
        )

        plt.clf()
        plt.plot(nn.prediction[:50], validation_training.T[1][:50])
        plt.plot(validation_target[:50], validation_training.T[1][:50])
        # plot 1/1 line
        plt.xlabel("IC")
        plt.ylabel("depth")
        plt.savefig("./tests/test_output/test_tutorial/comparison_depth.png")
